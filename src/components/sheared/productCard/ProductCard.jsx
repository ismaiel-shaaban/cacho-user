...
const ProductCard = ({ product }) => {
    ...
    const { image, discount, type, marketName, ratingCount, rating, title, price, oldPrice, link } = product;

    const discountLabel = discount ? (lang === "ar" ? `${strings.off} ${discount}` : `${discount} ${strings.off}`) : null;
    ...
    return (
        <Card dir={lang === "ar" ? "rtl" : "ltr"} className={`shadow-none !transition !duration-300 hover:shadow-xl`}>
            <CardBody className="p-0 overflow-hidden relative">
                ...
                {discountLabel && <div className={`${classes.discount} absolute -rotate-45 ${discountClass} text-white w-[300px] h-[50px] text-center flex items-center justify-center text-[14px]`}>{discountLabel}</div>}
                {typeLabel && <div className={`${classes.discount} absolute -rotate-45 ${discountClass} text-white w-[300px] h-[50px] text-center flex items-center justify-center text-[14px]`}>{typeLabel}</div>}
